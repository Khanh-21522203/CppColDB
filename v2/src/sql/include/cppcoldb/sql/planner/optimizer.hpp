...
// Rewrites a bound LogicalPlan via a fixed pipeline of optimization passes.
class Optimizer {
public:
...
private:
    ...
    void CollectColumnRefs(const LogicalExpr& e, std::set<std::size_t>& out);
    std::unique_ptr<LogicalPlan> PruneColumns(
        std::unique_ptr<LogicalPlan> plan,
        std::set<std::size_t>& required_cols);
